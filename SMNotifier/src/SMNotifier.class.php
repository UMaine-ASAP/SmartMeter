	...
	public function notify($id, $message, $emailSubject = null) {
		...
		if (is_int($id)) { // Id must be an int
			...
			if (is_array($id)) { // Try to provide some helpful hints for multiple user calls
				thow new SMNDeliveryError('Please enter a valid database user row id.  If you meant to notify multiple users, please use the SMNotifier.notifyMultiple() instead.', 11)
			} else {
